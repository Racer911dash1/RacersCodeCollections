#include "internal/rcc_mathcros_internal.h"

long long	__rcc_add_llong(long long a, long long b)
{
	return (a + b);
}

int8_t	__rcc_add_int8(int8_t a, int8_t b)
{
	return (a + b);
}

int16_t	__rcc_add_int16(int16_t a, int16_t b)
{
	return (a + b);
}

int32_t	__rcc_add_int32(int32_t a, int32_t b)
{
	return (a + b);
}

int64_t	__rcc_add_int64(int64_t a, int64_t b)
{
	return (a + b);
}

unsigned long long	__rcc_add_ullong(unsigned long long a, unsigned long long b)
{
	return (a + b);
}

uint8_t	__rcc_add_uint8(uint8_t a, uint8_t b)
{
	return (a + b);
}

uint16_t	__rcc_add_uint16(uint16_t a, uint16_t b)
{
	return (a + b);
}

uint32_t	__rcc_add_uint32(uint32_t a, uint32_t b)
{
	return (a + b);
}

uint64_t	__rcc_add_uint64(uint64_t a, uint64_t b)
{
	return (a + b);
}

float	__rcc_add_float(float a, float b)
{
	return (a + b);
}

double	__rcc_add_double(double a, double b)
{
	return (a + b);
}

long double	__rcc_add_ldouble(long double a, long double b)
{
	return (a + b);
}

long long	__rcc_subtract_llong(long long a, long long b)
{
	return (a - b);
}

int8_t	__rcc_subtract_int8(int8_t a, int8_t b)
{
	return (a - b);
}

int16_t	__rcc_subtract_int16(int16_t a, int16_t b)
{
	return (a - b);
}

int32_t	__rcc_subtract_int32(int32_t a, int32_t b)
{
	return (a - b);
}

int64_t	__rcc_subtract_int64(int64_t a, int64_t b)
{
	return (a - b);
}

unsigned long long	__rcc_subtract_ullong(unsigned long long a, unsigned long long b)
{
	return (a - b);
}

uint8_t	__rcc_subtract_uint8(uint8_t a, uint8_t b)
{
	return (a - b);
}

uint16_t	__rcc_subtract_uint16(uint16_t a, uint16_t b)
{
	return (a - b);
}

uint32_t	__rcc_subtract_uint32(uint32_t a, uint32_t b)
{
	return (a - b);
}

uint64_t	__rcc_subtract_uint64(uint64_t a, uint64_t b)
{
	return (a - b);
}

float	__rcc_subtract_float(float a, float b)
{
	return (a - b);
}

double	__rcc_subtract_double(double a, double b)
{
	return (a - b);
}

long double	__rcc_subtract_ldouble(long double a, long double b)
{
	return (a - b);
}

long long	__rcc_multiply_llong(long long a, long long b)
{
	return (a * b);
}

int8_t	__rcc_multiply_int8(int8_t a, int8_t b)
{
	return (a * b);
}

int16_t	__rcc_multiply_int16(int16_t a, int16_t b)
{
	return (a * b);
}

int32_t	__rcc_multiply_int32(int32_t a, int32_t b)
{
	return (a * b);
}

int64_t	__rcc_multiply_int64(int64_t a, int64_t b)
{
	return (a * b);
}

unsigned long long	__rcc_multiply_ullong(unsigned long long a, unsigned long long b)
{
	return (a * b);
}

uint8_t	__rcc_multiply_uint8(uint8_t a, uint8_t b)
{
	return (a * b);
}

uint16_t	__rcc_multiply_uint16(uint16_t a, uint16_t b)
{
	return (a * b);
}

uint32_t	__rcc_multiply_uint32(uint32_t a, uint32_t b)
{
	return (a * b);
}

uint64_t	__rcc_multiply_uint64(uint64_t a, uint64_t b)
{
	return (a * b);
}

float	__rcc_multiply_float(float a, float b)
{
	return (a * b);
}

double	__rcc_multiply_double(double a, double b)
{
	return (a * b);
}

long double	__rcc_multiply_ldouble(long double a, long double b)
{
	return (a * b);
}

long long	__rcc_divide_llong(long long a, long long b)
{
	return (a / b);
}

int8_t	__rcc_divide_int8(int8_t a, int8_t b)
{
	return (a / b);
}

int16_t	__rcc_divide_int16(int16_t a, int16_t b)
{
	return (a / b);
}

int32_t	__rcc_divide_int32(int32_t a, int32_t b)
{
	return (a / b);
}

int64_t	__rcc_divide_int64(int64_t a, int64_t b)
{
	return (a / b);
}

unsigned long long	__rcc_divide_ullong(unsigned long long a, unsigned long long b)
{
	return (a / b);
}

uint8_t	__rcc_divide_uint8(uint8_t a, uint8_t b)
{
	return (a / b);
}

uint16_t	__rcc_divide_uint16(uint16_t a, uint16_t b)
{
	return (a / b);
}

uint32_t	__rcc_divide_uint32(uint32_t a, uint32_t b)
{
	return (a / b);
}

uint64_t	__rcc_divide_uint64(uint64_t a, uint64_t b)
{
	return (a / b);
}

float	__rcc_divide_float(float a, float b)
{
	return (a / b);
}

double	__rcc_divide_double(double a, double b)
{
	return (a / b);
}

long double	__rcc_divide_ldouble(long double a, long double b)
{
	return (a / b);
}